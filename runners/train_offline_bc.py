...
def train_diffusion_bc(data_path, env_name="highway-v0", num_epochs=50, batch_size=256, learning_rate=3e-4, run_name=None):
    print("=" * 60)
    ...
    # 准备模型保存路径 (使用绝对路径构建)
    if run_name:
        save_dir = os.path.join(PROJECT_ROOT, "outputs", env_name, "models", run_name, "bc_pretrain")
    else:

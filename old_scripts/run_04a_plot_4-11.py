...
def smooth_curve(scalars, weight=0.98):
    """
...
def plot_training_curves(models_dict, save_dir, tags_to_plot, smooth_weight=0.85, align_to_env_steps=False, custom_limits=None):
    ...
    for target_tags, y_label in tags_to_plot:
        ...
        if has_data:
            ...
            # 将时空对齐的 Y 轴标签转化为论文所需的呈现格式
            display_y_label = "回合累积奖励" if y_label == "全局环境交互奖励" else y_label
            ...
            if custom_limits and y_label in custom_limits:
                custom_xlim = custom_limits[y_label].get("xlim")
                custom_ylim = custom_limits[y_label].get("ylim")

            # --- 智能应用 Y 轴极值 (最高优先级) ---
            if custom_ylim is not None:
                curr_ymin, curr_ymax = ax.get_ylim()
                ax.set_ylim(custom_ylim[0] if custom_ylim[0] is not None else curr_ymin,
                            custom_ylim[1] if custom_ylim[1] is not None else curr_ymax)
            elif "奖励" in y_label or "步数" in y_label or "Q值" in y_label:
                # 限定上限为“所有平滑曲线最高均值的 1.2 倍”，彻底拒绝被偶尔失控的背景阴影拉伸 Y 轴
                dynamic_top = global_max_smoothed * 1.20
                if dynamic_top > 0:
                    ax.set_ylim(0, dynamic_top)
                else:
                    ax.set_ylim(0, None)
            ...
            if custom_xlim is not None:
                ...
                ax.set_xlim(custom_xlim[0] if custom_xlim[0] is not None else curr_xmin,
                            custom_xlim[1] if custom_xlim[1] is not None else curr_xmax)
            else:
...
if __name__ == "__main__":
    ...
    if TARGET_ENV == "merge-v0":
        models_to_plot = {
            ...
            "Diff-SAC 崩塌 (DM08)": f"outputs/{TARGET_ENV}/logs/DiffSAC_DM08_Mixed_Strong_Q_20260513_211948",
        }
        ...
        models_to_plot = {
            ...
            #"Diff-SAC 终极SOTA (DR02)": f"outputs/{TARGET_ENV}/logs/DiffSAC_DR02_Micro_Q_20260510_060657",
            #"Diff-SAC 终极SOTA (DR04)": f"outputs/{TARGET_ENV}/logs/DiffSAC_DR04_Strong_Q_20260510_123826",
    ...
    save_directory = os.path.join(PROJECT_ROOT, "old_scripts", "output_plot", "plot4-11", folder_name)
